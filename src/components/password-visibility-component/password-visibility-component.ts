...
export class PasswordVisibilityComponent extends Component {
  private _visible: HTMLSpanElement
  private _invisible: HTMLSpanElement
  private _target: any
  ...
  protected _getVisible() {
    return this._visible
  }

  protected _getInvisible() {
    return this._invisible
  }

  protected _createVisible() {
    const visible: HTMLSpanElement = document.createElement('span')
  ...
  protected _createInvisible() {
    const invisible: HTMLSpanElement = document.createElement('span')
  ...
  protected _initialize() {
    ...
    const targetComponent = Venom.getComponent(target)

    // @ts-ignore
    if (targetComponent) this._target = targetComponent
  ...
  protected _onChangeVisibility = (): void => {
    const type: string = this._target.getType(),
      isVisible: boolean = type === 'text'

    this._target.setType(isVisible ? 'password' : 'text')

    if (isVisible) {
      this._getInvisible().classList.add(HIDDEN_MODIFIER)
      this._getVisible().classList.remove(HIDDEN_MODIFIER)
    } else {
      this._getVisible().classList.add(HIDDEN_MODIFIER)
      this._getInvisible().classList.remove(HIDDEN_MODIFIER)
    }
  }
}
