...
export class PasswordVisibilityComponent {
  ...
  createInvisible() {
    ...
    invisible.classList.add(HELPER_CLASSNAME)
  ...
  initialize(component) {
    ...
    this.target = Venom.getComponent(target).getComponent()

    this.createVisible()
    this.createInvisible()

    this.getElement().addEventListener('click', this.onChangeVisibility)
  }
  ...
  onChangeVisibility = (event) => {
    ...
    this.getTarget().setType(isVisible ? 'password' : 'text')

    if (isVisible) {
      this.getInvisible().classList.add(HELPER_CLASSNAME)
      this.getVisible().classList.remove(HELPER_CLASSNAME)
    } else {
      this.getVisible().classList.add(HELPER_CLASSNAME)
      this.getInvisible().classList.remove(HELPER_CLASSNAME)
    }
  }
