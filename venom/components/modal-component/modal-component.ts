...
export class ModalComponent extends Component {
  ...
  setFooterContent(content: any): this {
    this.getFooter().innerHTML = ""
    if (typeof content === 'string') {
      this.getFooter().innerHTML = content
    } else {
      this.getFooter().appendChild(content)
    }
    return this
  }

  setFooterContentFromTemplate(id: string): this {
    const template: HTMLElement = document.getElementById(id)
    // @ts-ignore
    this.setFooterContent('')
    this.getFooter().appendChild(document.importNode((template as any).content, true))
    return this
  }

  setFooterContentFromData(attribute: string, value: string, multiple: boolean = false): this {
    const param = `[data-${attribute}=${value}]`,
      template = multiple ? [].slice.call(document.querySelectorAll(param)) : document.querySelector(param)
    ...
    // @ts-ignore
    if (!multiple) this.getFooter().appendChild(document.importNode(template.content, true))

    // @ts-ignore
    if (template) template.map((t) => this.getFooter().appendChild(document.importNode(t.content, true)))

    return this
  }

  getFooter(): HTMLDivElement {
    if (!this._footer) this._footer = this.getElement().querySelector(FOOTER)
    return this._footer
  }
  ...
  changeClose(callback: Function): this {
    ...
    if (this._callback) this.resetClose()
  ...
  resetClose() {
    var el = this.getElement().querySelector(CLOSE_BUTTON)
